...
if opt.makedata:
    ...
    data['metho'] = None
    data['concl'] = None
    tex_data = list(deepcopy(data['tex_data'].values))
...
test_iter = BucketIterator(
    test, batch_size=opt.batch_size, train=False, shuffle=False)
...
if not opt.notrain:
    ...
    for epoch in range(opt.num_epochs):
        ...
        for batch in tqdm(train_iter):
            text = {'abstr':batch.abstr, 'title':batch.title, 'intro':batch.intro, 'relat':batch.relat, 'metho':batch.metho, 'concl':batch.concl}
